...
def ffind_calibration_angular():
    ...
    index = np.argmin(np.abs(transmissions - 0.1))
    for i in angles_urad:
        ...
        cal_curve.append(sample_thick_interp[index])
    coeffs = np.polyfit(angles_urad,cal_curve,4)
    with h5py.File('Beam_Hardening_LUT_Raytheon.hdf5','r+') as hdf_file:
...
def fsave_coeff_data():
    ...
    with h5py.File('Beam_Hardening_LUT_Raytheon.hdf5','w') as hdf_file:
        ...
        hdf_file.attrs['Notes'] = '''Created by Alan Kastengren, May 2, 2018.
    LUT between transmission and thickness of Inconel 625, using the BM spectrum with 
    750 um of Be, 500 um Ge, and 250 um Cu.'''

def fconvert_data(input_trans):
    with h5py.File('Beam_Hardening_LUT_Raytheon.hdf5','r') as hdf_file:
        fit_coeffs = hdf_file['Fit_Coeff']
    return np.polyval(fit_coeffs,np.log(input_trans))

def fcorrect_angular(pathlength_image,center_row,d_source_m,pixel_size_mm):
    '''Corrects for
    '''
    with h5py.File('Beam_Hardening_LUT_Raytheon.hdf5','r') as hdf_file:
        coeff = hdf_file['Angular_Fits'][...] 
    angles = np.abs(np.arange(pathlength_image.shape[0]) - center_row)
    angles *= pixel_size_mm / d_source_m * 1e3
    correction_factor = np.polyval(coeff,angles)
...
def fmake_plots():
    ...
    with h5py.File('Beam_Hardening_LUT_Raytheon.hdf5','w') as hdf_file:
        hdf_file.create_dataset('Transmission',data=transmissions)
        hdf_file.create_dataset('Microns_Fe',data=sample_thick_interp)
